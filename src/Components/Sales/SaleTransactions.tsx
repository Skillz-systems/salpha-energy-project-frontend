...
const SaleTransactions = ({
  ...
  const handleCompletePayment = async (paymentId: string) => {
    ...
    if (paymentSummary.isInstallment) {
      // For installments, suggest remaining balance or a reasonable installment amount
      const suggestedAmount = Math.min(
        paymentSummary.remainingBalance, 
        Math.max(paymentSummary.totalAmount * 0.1, 10000) // 10% of total or minimum ₦10,000
      );
