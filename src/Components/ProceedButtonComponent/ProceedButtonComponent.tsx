...
const ProceedButton: React.FC<ButtonProps> = ({
  ...
  const variantClasses = {
    yellow:
      "bg-[#FEF5DA] border border-[#A58730]/20 shadow-[2px_6px_8px_0px_rgba(0,0,0,0.15)] hover:bg-[#F8CB48] hover:border-[#A58730]/20",
    gray: "bg-[#E2E4EB] border border-[#9BA4BA]/20",
    gradient: "bg-[#FEF5DA] border border-[#A58730]/20 shadow-innerCustom hover:bg-[#F8CB48]",
    red: "bg-[#941C12] border border-[#63130C]/20 hover:bg-[#941C12]",
  };

  // Determine arrow color based on state
  const getArrowColor = () => {
    if (isHovered) return "white"; // White on hover for all variants
    if (variant === "red") return "white"; // White for red variant
    return "#828DA9"; // Default color
  };
    ...
    <button
      ...
      className={`flex items-center justify-center w-16 h-16 rounded-full transition-all ${
        isHovered 
          ? variant === "red" 
            ? variantClasses.red 
            : "bg-[#F8CB48] border border-[#A58730]/20"
